...
if mh.checkstate('delay_state')
    ...
    mh.Screen('FillOval', mh, mh.trialtarg('T0','getcolor') , mh.trialtarg('T0','getpos'));
    mh.Screen('FillOval', mh, mh.trialtarg('T1_Moving_out','getcolor'),mh.trialtarg('T1_Moving_out','getpos'));
    % mh.plotwindow('T0',mh.trialtarg('T0','getpos'));
    % mh.plotwindow('T1_Moving_out',mh.trialtarg('T1_Moving_out','getpos'));
...
if mh.checkstate('T1_reach_state_moving')
    targpos=mh.trialtarg('T1_Moving_out','getpos','continue','delay_state');
    mh.Screen('FillOval', mh, mh.trialtarg('T1_Moving_out','getcolor'),targpos);
    % mh.plotwindow('T1_Moving_out',targpos);

    if mh.checkint('T1_reach_state_moving','T1_Reach') && ~mh.checkeye('T1_Moving_out',targpos)
        % just chill
    elseif ~mh.checkeye('T1_Moving_out',targpos)
        mh.stoptrial(0);
    elseif mh.checkint('T1_reach_state_moving','T1_Reach') && mh.checkeye('T1_Moving_out',targpos)
...
if mh.checkstate('T1_hold_state_moving')
    ...
    if mh.checkint('T1_hold_state_moving','T1_Hold') && mh.checkeye('T1_Moving_out',targpos)
        % just chill
    elseif ~mh.checkeye('T1_Moving_out',targpos)
        mh.stoptrial(0);
    else %set conditions for continuing
