...
if mh.checkstate('delay_state')
    ...
    mh.Screen('FillOval', mh, mh.trialtarg('T0','getcolor') , mh.trialtarg('T0','getpos'));
    mh.Screen('FillOval', mh, mh.trialtarg('T1_Moving_CW','getcolor'),mh.trialtarg('T1_Moving_CW','getpos'));
...
if mh.checkstate('T1_reach_state_moving')
    targpos=mh.trialtarg('T1_Moving_CW','getpos','continue','delay_state');
    mh.Screen('FillOval', mh, mh.trialtarg('T1_Moving_CW','getcolor'),targpos);

    if mh.checkint('T1_reach_state_moving','T1_Reach') && ~mh.checkeye('T1_Moving_CW',targpos)
        % just chill
    elseif ~mh.checkeye('T1_Moving_CW',targpos)
        mh.stoptrial(0);
    elseif mh.checkint('T1_reach_state_moving','T1_Reach') && mh.checkeye('T1_Moving_CW',targpos)
...
if mh.checkstate('T1_hold_state_moving')
    ...
    if mh.checkint('T1_hold_state_moving','T1_Hold') && mh.checkeye('T1_Moving_CW',targpos)
        % just chill
    elseif ~mh.checkeye('T1_Moving_CW',targpos)
        mh.stoptrial(0);
    else %set conditions for continuing
